  ...
  return (
    <>
      ...
      <div className='w-[90%] h-[65%] bg-white text-black rounded-lg shadow-black shadow-2xl ring ring-gray-300'>
        <form onSubmit={handleSubmit(submitHandler)} className='w-full h-full flex flex-col gap-5 px-6 py-4'>
          <h1 className='text-2xl text-center'>Seller Login</h1>
          <label htmlFor="email">
            email:-
            <input type="email" {...register("email", { required: true })}className='w-full mt-1 text-black rounded-lg outline-none border-1 border-gray-400 px-2 py-2'/>
            {errors.email && <span className='text-red-700 leading-0 text-xs'>This field is required</span>}
          </label>
          <label htmlFor="pvssword">
            password:-
            <input type="password" {...register("password", { required: true })} className='w-full mt-1 text-black rounded-lg outline-none border-1 border-gray-400 px-2 py-2'/>
